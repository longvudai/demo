...
extension Calendar {
    public func weekdaySymbol(from weekday: WeekDay) -> String {
        let weekDayRaw = weekday.weekDayRaw(from: self)
        return weekdaySymbols[weekDayRaw - 1]
    }

    public func shortWeekDaySymbol(from weekday: WeekDay) -> String {
        let weekDayRaw = weekday.weekDayRaw(from: self)
        return shortWeekdaySymbols[weekDayRaw - 1]
    }

    public func veryShortWeekDaySymbol(from weekday: WeekDay) -> String {
        let weekDayRaw = weekday.weekDayRaw(from: self)
...
extension Calendar {
    public static func current(firstDayOfWeek: WeekDay) -> Calendar {
